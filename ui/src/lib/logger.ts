...
class Logger {
  ...
  private formatMessage(level: LogLevel, message: string, context?: LogContext, error?: Error, data?: unknown): LogEntry {
  ...
  debug(message: string, context?: LogContext, data?: unknown): void {
    if (this.shouldLog(LogLevel.DEBUG)) {
      this.output(this.formatMessage(LogLevel.DEBUG, message, context, undefined, data));
    }
  }

  info(message: string, context?: LogContext, data?: unknown): void {
    if (this.shouldLog(LogLevel.INFO)) {
      this.output(this.formatMessage(LogLevel.INFO, message, context, undefined, data));
    }
  }

  warn(message: string, context?: LogContext, data?: unknown): void {
    if (this.shouldLog(LogLevel.WARN)) {
      this.output(this.formatMessage(LogLevel.WARN, message, context, undefined, data));
    }
  }

  error(message: string, error?: Error, context?: LogContext, data?: unknown): void {
    if (this.shouldLog(LogLevel.ERROR)) {
      this.output(this.formatMessage(LogLevel.ERROR, message, context, error, data));
    }
  }
  ...
  time<T>(label: string, fn: () => T, context?: LogContext): T {
    ...
    try {
      ...
      this.debug(`✅ Completed: ${label}`, { ...context, duration: `${duration.toFixed(2)}ms` });
      return result;
    } catch (error) {
      const duration = performance.now() - start;
      this.error(`❌ Failed: ${label}`, error as Error, { ...context, duration: `${duration.toFixed(2)}ms` });
      throw error;
    }
  }
  ...
  async timeAsync<T>(label: string, fn: () => Promise<T>, context?: LogContext): Promise<T> {
    ...
    try {
      ...
      const duration = performance.now() - start;
      this.debug(`✅ Completed async: ${label}`, { ...context, duration: `${duration.toFixed(2)}ms` });
      return result;
    } catch (error) {
      const duration = performance.now() - start;
      this.error(`❌ Failed async: ${label}`, error as Error, { ...context, duration: `${duration.toFixed(2)}ms` });
      throw error;
    }
  }

  // Log entry and exit of a function
  trace<T>(functionName: string, fn: () => T, context?: LogContext, logArgs?: unknown): T {
    this.debug(`🔵 Entering: ${functionName}`, context, logArgs);
    
    try {
      const result = fn();
      this.debug(`🟢 Exiting: ${functionName}`, context, typeof result === 'object' ? result : { result });
      return result;
    } catch (error) {
  ...
  async traceAsync<T>(functionName: string, fn: () => Promise<T>, context?: LogContext, logArgs?: unknown): Promise<T> {
    this.debug(`🔵 Entering async: ${functionName}`, context, logArgs);
    
    try {
      const result = await fn();
      this.debug(`🟢 Exiting async: ${functionName}`, context, typeof result === 'object' ? result : { result });
      return result;
    } catch (error) {
...
export const logTransaction = (hash: string, message: string, context?: LogContext) => {
  logger.info(`🔗 Transaction: ${message}`, { ...context, transaction: hash });
};

export const logContractCall = (contract: string, method: string, args?: unknown, context?: LogContext) => {
  logger.debug(`📞 Contract call: ${contract}.${method}`, { ...context, contract, method }, args);
};

export const logWalletEvent = (event: string, address?: string, context?: LogContext) => {
...
export const logApiCall = (url: string, method: string = 'GET', context?: LogContext) => {
